...
Meteor.methods({
  ...
  images(folder, pageIndex, pageLength) {
    ...
    const getFolderDesc = (path) => {
      return {
        name: basename(path),
        url:
          `/browse/${pageIndex}/${pageLength}/` +
          encodeURIComponent(folderSlash + path),
      };
    };
    ...
    if (existing && !isDirectory(leaf)) {
      return {
        error:
          leaf +
          " is a file but should be a folder. Check the documentation and your settings.json",
      };
    }
    if (!existing) {
      return {
        error:
          leaf +
          " does not exists. Check the documentation and your settings.json",
      };
    }
  ...
  saveData(sample) {
    ...
    if (sample.tags) {
      SseProps.upsert(
        { key: "tags" },
        { $addToSet: { value: { $each: sample.tags } } }
      );
    }
