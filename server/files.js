...
Meteor.startup(() => {
  ...
  WebApp.connectHandlers.use(
    "/file",
    serveStatic(imagesFolder, { fallthrough: false })
  );
  WebApp.connectHandlers.use(
    "/datafile",
    serveStatic(pointcloudsFolder, { fallthrough: true })
  );
  ...
  WebApp.connectHandlers.use(
    bodyParser.raw({ limit: "200mb", type: "application/octet-stream" })
  );
  WebApp.connectHandlers.use("/save", function (req, res) {
    if (demoMode) return;
    const fileToSave =
      pointcloudsFolder + decodeURIComponent(req.url).replace("/save", "");
