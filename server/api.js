...
WebApp.connectHandlers.use(
  "/api/pcdtext",
  generatePCDOutput.bind({ fileMode: false })
);
WebApp.connectHandlers.use(
  "/api/pcdfile",
  generatePCDOutput.bind({ fileMode: true })
);
...
function generatePCDOutput(req, res, next) {
  ...
  const objectFile =
    pointcloudsFolder + decodeURIComponent(req.url) + ".objects";

  if (this.fileMode) {
    res.setHeader(
      "Content-disposition",
      "attachment; filename=DOC".replace("DOC", fileName)
    );
  ...
  readFile(pcdFile, (err, content) => {
    ...
    out +=
      "POINTS " + pcdContent.header.width * pcdContent.header.height + "\n";
        ...
        pcdContent.position.forEach((v, i) => {
          ...
          switch (i % 3) {
            ...
            case 2:
              ...
              if (assignedObject != undefined) out += assignedObject;
              else out += "-1";
              out += "\n";
              res.write(out);
              out = "";
