  ...
  constructor(THREE) {
    ...
    THREE.PCDLoader.prototype = {
      ...
      parse: function (data, url) {
        ...
        var textData = this.serverMode
          ? Buffer.from(data).toString()
          : THREE.LoaderUtils.decodeText(data);
        ...
        if (PCDheader.data === "ascii") {
          ...
          let camPosition = new THREE.Vector3(
            parseFloat(meta.viewpoint.tx),
            parseFloat(meta.viewpoint.ty),
            parseFloat(meta.viewpoint.tz)
          );
          let camQuaternion = new THREE.Quaternion(
            meta.viewpoint.qx,
            meta.viewpoint.qy,
            meta.viewpoint.qz,
            meta.viewpoint.qw
        ...
        if (PCDheader.data === "binary_compressed") {
          ...
          let camPosition = new THREE.Vector3(
            parseFloat(PCDheader.viewpoint.tx),
            parseFloat(PCDheader.viewpoint.ty),
            parseFloat(PCDheader.viewpoint.tz)
          );
          let camQuaternion = new THREE.Quaternion(
            PCDheader.viewpoint.qx,
            PCDheader.viewpoint.qy,
            PCDheader.viewpoint.qz,
            PCDheader.viewpoint.qw
          );
          ...
          for (var i = 0; i < PCDheader.points; i++) {
            ...
            if (offset.label !== undefined) {
              const classIndex = dataview.getUint8(
                PCDheader.points * offset.label + PCDheader.size[3] * i
              );
              item.classIndex = classIndex;
              label.push(classIndex);
            } else {
        ...
        if (label.length > 0)
          geometry.setAttribute(
            "label",
            new THREE.Uint8BufferAttribute(label, 3)
          );
        if (color.length > 0) {
