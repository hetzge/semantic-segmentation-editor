...
export default class SseEditor3d extends React.Component {
  constructor() {
    ...
    this.pointSizeWithAttenuation = 0.03;
    ...
    this.cameraTween = new TWEEN.Tween(this.cameraState)
      .onStop(endCameraTween)
      .onComplete(endCameraTween);
  ...
  setColor(idx, color = { red: 1, green: 0, blue: 0 }) {
    ...
    const altColor = this.getCachedColor(
      color.red,
      color.green,
      color.blue,
      this.colorBoost
  ...
  changeClassOfSelection(classIndex) {
    ...
    if (this.selectedObject) {
      ...
      this.selectedObject.points.forEach((idx) =>
        this.assignNewClass(idx, classIndex)
      );
  ...
  createObject() {
    let points;
    if (this.selection.size > 0) points = this.selection;
    else if (this.viewFilterState === "points") points = this.visibleIndices;
    if (points) {
      const obj = {
        id: Random.id(),
  ...
  invalidateCounters() {
    ...
    this.activeSoc.labels.forEach((a) => {
      const arg = {
        classIndex: i,
        count: (this.cloudData.byClassIndex[i] || { size: 0 }).size,
      };
  ...
  removeObjectPoints() {
    ...
    this.selection.forEach((idx) => {
      if (ptsArray.indexOf(idx) != -1) {
        ...
        if (this.autoFilterMode) this.hideIndex(idx);
      }
    });
    this.clearSelection();
    this.saveAll();
  ...
  componentDidMount() {
    ...
    const changePointSize = (amount) => {
      const withAttenuation = { min: 0.01, max: 0.5, increment: 0.01 };
      const withoutAttenuation = { min: 1, max: 5, increment: 0.5 };
      const bounds = this.cloudObject.material.sizeAttenuation
        ? withAttenuation
        : withoutAttenuation;
      this.cloudObject.material.size =
        bounds.min + amount * (bounds.max - bounds.min);
  ...
  init() {
    ...
    this.orbiter = new THREE.OrbitControls(
      this,
      this.camera,
    ...
    this.orbiter.addEventListener(
      "change",
      this.orbiterChange.bind(this),
      false
    );
  ...
  cameraPreset(where, bypassCameraPresetInfo = false) {
    ...
    if (eye) this.fitView(this.visibleIndices, eye);
  }
  ...
  moveCamera(eye, target) {
    ...
    let eye1FromTarget;
    if (eye)
      eye1FromTarget = new THREE.Spherical()
        .setFromVector3(eye.sub(target1))
        .makeSafe();
    else eye1FromTarget = eye0FromTarget;
  ...
  selectByPolygon(polygon) {
    ...
    if (this.autoFilterMode && !this.viewFilterState) {
      if (inside.size > 0) {
        ...
        if (this.autoFocusMode) this.subsetFocus(inside);
    ...
    if (this.selection.size > 0 && this.selectedObject)
      this.sendMsg("enableCommand", { name: "addPointsObjectCommand" });
    else this.sendMsg("disableCommand", { name: "addPointsObjectCommand" });
    this.updateGlobalBox();
  ...
  processSelection(idx) {
    if (this.selectionMode == "add") {
      ...
      if (this.selection.has(idx)) this.removeIndexFromSelection(idx);
      else {
        this.addIndexToSelection(idx);
      }
    }
  ...
  originalCoordinates(idx) {
    ...
    geometry
      .rotateZ(this.meta.rotationZ || 0)
      .rotateY(this.meta.rotationY || 0)
      .rotateX(this.meta.rotationX || 0);
  ...
  updatePixelProjection(idx) {
    ...
    const toScreen = (item, idx) => {
      ...
      if (inFrustrum) this.frustrumIndices.add(idx);
      else {
        this.frustrumIndices.delete(idx);
      }
      if (inFrustrum || this.selection.has(idx)) {
        vector.project(this.camera);
        projection.pixelX = Math.round(
          vector.x * this.viewWidth2 + this.viewWidth2
        );
        projection.pixelY = Math.round(
          -vector.y * this.viewHeight2 + this.viewHeight2
        );
      } else {
    ...
    if (idx !== undefined) return toScreen(this.cloudData[idx], idx);
    else
      this.cloudData.forEach((pt, idx) => {
        toScreen(pt, idx);
      });
  ...
  resizeCanvas() {
    ...
    if (this.orbiter && this.orbiter.handleResize) this.orbiter.handleResize();
  }
  ...
  _orientationRaycasting() {
    ...
    if (this.mouse.dragged > 0) return;
  ...
  _raycasting() {
    if (!this.cloudObject || !this.raycastingIsDirty) return;
    this.raycaster.setFromCamera(this.mouse, this.camera);
  ...
  drawCanvasMouse() {
    if (!this.canvasMouseIsDirty) return;
    ...
    if (
      ...
      const drawCircle = (x, y, r) => {
        ...
        for (let i = 0; i <= total; i++) {
          pts.push([
            x + Math.cos((DOUBLEPI * i) / total) * r,
            y + Math.sin((DOUBLEPI * i) / total) * r,
          ]);
        }
        return pts;
      };
      ...
      this.drawPolyLine(
        ...
        PolyBool.polygon(segments).regions[0],
        "yellow",
        0,
        1,
        true
  ...
  animate(time) {
    if (this.sendMsg) requestAnimationFrame(this.animate.bind(this));
    else return;
    ...
    if (this.orbiting || justZoom) {
      ...
      if (this.mouse.dragged == 0) this.orbiting = false;
      ...
      if (this.pixelProjectionRequestTime) {
        ...
        if (this.orientationArrows.size > 0) this._orientationRaycasting();
        else {
          this._raycasting();
        }
      }
  ...
  rotateGeometry(rx, ry, rz) {
    ...
    this.cloudGeometry
      .rotateX(this.meta.rotationX)
      .rotateY(this.meta.rotationY)
      .rotateZ(this.meta.rotationZ);
    this.display(
      this.objects,
      this.positionArray,
      this.labelArray,
  ...
  startPointcloudOrientation() {
    if (this.orientationArrows.size > 0) return;
    this.cameraPreset("front", true);
    this.resetRotation();
  ...
  setupRaycaster(boundingBox, numberOfPoints) {
    ...
    const threshold =
      Math.cbrt(
        (boundingBox.x * boundingBox.y * boundingBox.z) / numberOfPoints
      ) / 3;
  ...
  drawBSphere(arr) {
    ...
    let pts = [];
    if (arr) arr.forEach((idx) => pts.push(this.cloudData[idx]));
    else arr = this.cloudData;
    ...
    if (this.globalSphereObject) this.scene.remove(this.globalSphereObject);
  ...
  activateTool(tool) {
    ...
    if (tool.activate) tool.activate.bind(tool)();
  ...
  changeTarget(ev) {
    if (this.mouseTargetIndex) {
      const newTarget = new THREE.Vector3().copy(
        this.cloudData[this.mouseTargetIndex]
      );
    ...
    if (this.mouse.dragged < 4 && this.pendingOrientationArrow) {
      ...
      if (this.pendingUpVector) {
        ...
        this.sendMsg("alert", {
          autoHide: false,
          message:
            "Point cloud orientation: Now select the arrow which points to the FRONT",
          buttonText: "CANCEL",
          closeMessage: "orientation-abort",
          forceCloseMessage: "orientation-close",
        });
  ...
  mouseMove(iev) {
    ...
    if (this.mouse.down) this.mouseDrag(ev);
  ...
  display(objectArray, positionArray, labelArray, rgbArray) {
    return new Promise((res, rej) => {
      ...
      this.cloudData.reduce((acc, cur) => {
        if (acc[cur.classIndex]) acc[cur.classIndex].add(cur);
        else acc[cur.classIndex] = new Set([cur]);
        return acc;
      }, this.cloudData.byClassIndex);
  ...
  loadPCDFile(fileUrl) {
    setTimeout(
      () =>
        this.sendMsg("bottom-right-label", { message: "Loading PCD data..." }),
      1
    );
  ...
  saveBinaryLabels() {
    this.dataManager.saveBinaryFile(
      this.props.imageUrl + ".labels",
      this.cloudData.map((x) => x.classIndex)
    );
  }

  saveBinaryObjects() {
    this.dataManager.saveBinaryFile(
      this.props.imageUrl + ".objects",
  ...
  start() {
    ...
    this.loadPCDFile(fileUrl).then(() => {
      ...
      this.dataManager
        .loadBinaryFile(this.props.imageUrl + ".labels")
        .then(
          (result) => {
            this.sendMsg("bottom-right-label", {
        ...
        .then(() => {
          this.dataManager
            .loadBinaryFile(this.props.imageUrl + ".objects")
            .then(
              (result) => {
                if (!result.forEach) result = undefined;
                this.display(
                  result,
                  this.positionArray,
                  this.labelArray,
