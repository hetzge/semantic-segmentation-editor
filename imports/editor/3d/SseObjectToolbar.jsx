...
class SseObjectButton extends React.Component {
  ...
  positionIndicator(backgroundColor = "#56FE45") {
    ...
    return <div style={indicatorStyle} className="position-indicator"></div>;
  }
  ...
  render() {
    ...
    return (
      <div
        className={"object-button" + (this.state.selected ? " selected" : "")}
      >
...
export default class SseObjectToolbar extends SseToolbar {
  ...
  updateState() {
    ...
    if (ssize && this.selectedObject) {
      ...
      const removeButtonVisible =
        selectionArray.find((idx) => objPoints.has(idx)) != undefined;
  ...
  messages() {
    ...
    this.onMsg("object-select", (arg) => {
      this.setState({
        objectSelected: arg.value,
        pointCount: arg.value ? arg.value.points.length : 0,
      });
  ...
  renderSelection() {
    ...
    let textColor = "white";
    if (this.state.soc) {
      const props = this.state.soc.propsForIndex(
        this.state.objectSelected.classIndex
      );
      label = props.label;
      color = props.color;
    ...
    return (
      <div className="vflex">
        <div className="hflex">
          ...
          >
            {label + " (" + this.state.pointCount + " pts)"}
          </div>
          {
            <div
              style={{
            ...
            >
              {this.renderMiniCommand("unselectObjectCommand")}
            </div>
          }
        </div>
        {!this.state.removeButtonVisible && !this.state.addButtonVisible ? (
          <div className="hflex">
            {this.renderMiniCommand(
              "deleteObjectCommand",
              "object-buttons grow"
            )}
            {this.renderMiniCommand(
  ...
  render() {
    ...
    return (
      <div className="sse-object-toolbar hflex no-shrink">
        ...
        <div
          className="object-list scroller group vflex grow wrap"
          onWheel={(ev) => this.wheelScrolling(ev)}
        >
          {Array.from(this.state.objects).map((o, idx) => (
            <SseObjectButton key={"_" + idx} soc={this.state.soc} object={o} />
          ))}
        </div>
      </div>
    );
