...
export default class SseUndoRedo2d {
  ...
  init(url, initialState) {
    ...
    if (infos) this.currentInfos = infos;
    else {
  ...
  undo() {
    if (this.currentInfos.undoArray.length == 1) return undefined;
  ...
  redo() {
    if (this.currentInfos.redoArray.length == 0) return undefined;
  ...
  updateAvailability() {
    ...
    else this.sendMsg("disableCommand", { name: "undoCommand" });
    if (this.currentInfos.redoArray.length > 0)
      this.sendMsg("enableCommand", { name: "redoCommand" });
    else this.sendMsg("disableCommand", { name: "redoCommand" });
  }
}
