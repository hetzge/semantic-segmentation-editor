...
export default class SseLayers extends React.Component {
  ...
  componentDidMount() {
    ...
    this.onMsg("sse-polygon-select", (arg) => {
      if (arg && arg.polygon) {
        ...
        this.setState({
          polygonLayer:
            arg.polygon != undefined ? arg.polygon.feature.layer : undefined,
        });
      } else {
  ...
  toggle(idx) {
    let visible = (this.state.layers[
      this.state.layers.length - 1 - idx
    ].visible = !this.state.layers[this.state.layers.length - 1 - idx].visible);
    this.setState(this.state);
    if (visible) this.sendMsg("layer-show", { index: idx });
    else this.sendMsg("layer-hide", { index: idx });
  }
  ...
  changeLayer(idx) {
    this.sendMsg("polygon-set-layer", {
        ...
        {this.state.layers.map((obj) => (
          ...
          >
            <div className={this.state.selected == obj.index ? "selected" : ""}>
              <div
                onClick={() => this.toggle(obj.index)}
                className="sse-layer-eye"
              >
              ...
              >
                <div className="p5 grow">
                  {obj.label + (obj.count ? " (" + obj.count + ")" : "")}
                </div>
              </div>
              {this.state.polygonLayer != undefined &&
              this.state.polygonLayer != obj.index ? (
                <div
                  onClick={() => {
