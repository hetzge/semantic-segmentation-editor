...
export default class SseGeometry {
  ...
  segmentAsString(seg) {
    ...
    return JSON.stringify({
      x: Math.round(seg.point.x * p),
      y: Math.round(seg.point.y * p),
    });
  }
  ...
  contiguousOutlinePoints(seg) {
    const contiguousOutlinePointsLocal = (seg) => {
      ...
      let previousPolygons =
        this.includedInPolygons.get(previousPoint) || new Set();

      let interNext = pointPolygons.intersection(nextPolygons);
      let interPrevious = pointPolygons.intersection(previousPolygons);

      let res = [];
      if (interNext.size <= 1) res.push(nextPoint);

      if (interPrevious.size <= 1) res.push(previousPoint);

      return new Set(res);
    };
    return Array.from(this.isoSegmentsSet(seg)).reduce(
      (acc, cur) => acc.union(contiguousOutlinePointsLocal(cur)),
      new Set()
    );
  }
  ...
  normalizePaperGraph(polygons) {
    ...
    polygons.forEach((pol) => {
      pol.segments.forEach((seg) => {
        ...
        this.nextOutlinePoints.set(
          pt,
          (nexts || new Set()).union(this.contiguousOutlinePoints(seg))
        );
      });
    });
    ...
    if (nextP1 && nextP1.size > 0) {
      if (nextP1.has(p2)) {
        ...
        return nextP1.forEach((p) => {
          if (!path.has(p))
            return this.findPathRec(
              p,
              p2,
              path.union(new Set([p])),
              result,
              length,
              p1
            );
  ...
  findIntersectingPolygons(path) {
    return this.polygons.filter(
      (p) => p.getIntersections(path, (cand) => cand != p).length > 0
    );
  }
