...
export default class SseEditor2d extends React.Component {
  ...
  resizeCanvas() {
    ...
    if (!canvasContainer || !this.raster) return;
    ...
    const gutterRatio = 0.98;
    if (viewRatio < bitmapRatio) {
      scaleFactor =
        (gutterRatio * canvasContainer.offsetWidth) / this.imageWidth;
    } else {
      scaleFactor =
        (gutterRatio * canvasContainer.offsetHeight) / this.imageHeight;
    }
    ...
    const fullScreenMatrix = new Paper.Matrix()
  ...
  keepPointInsideRaster(pt) {
    ...
    if (px < x) px = x;
    else if (px > x + w) px = x + w;
    if (py < y) py = y;
    else if (py > y + h) py = y + h;
  ...
  mergePath() {
    if (this.selectedIntersections) {
      if (this.selectedIntersections.size > 1) {
        this.sendMsg("alert", {
          message: "Click on a polygon that overlaps the selected polygon.",
        });
  ...
  mergePaths(p1, p2) {
    ...
    if (newPath.segments) {
      ...
      this.sendMsg("alert", {
        message:
          "Merging cancelled: The resulting polygon can not contain hole(s).",
      });
    }
  }
  ...
  updateCommands() {
    ...
    else this.sendMsg("disableCommand", { name: "enterCommand" });

    if (this.pathToFollowInfos)
      this.sendMsg("enableCommand", { name: "followCommand" });
    else this.sendMsg("disableCommand", { name: "followCommand" });

    if (this.actualSelection.length > 0)
      this.sendMsg("enableCommand", { name: "deleteCommand" });
    else this.sendMsg("disableCommand", { name: "deleteCommand" });

    if (this.selectedIntersections) {
      const sp = this.getSelectedPolygons()[0];
      const up = Array.from(this.selectedIntersections).some((p) =>
        sp.isBelow(p)
      );
      const dw = Array.from(this.selectedIntersections).some((p) =>
        sp.isAbove(p)
      );

      if (up) this.sendMsg("enableCommand", { name: "upCommand" });
      else this.sendMsg("disableCommand", { name: "upCommand" });
      if (dw) this.sendMsg("enableCommand", { name: "downCommand" });
      else this.sendMsg("disableCommand", { name: "downCommand" });
  ...
  drawPointSelection() {
    ...
    if (segments.length > 0) {
      ...
      segments.forEach((s) => {
        const pt = s.point;
        const pointIndicator = new Paper.Path.Circle(
  ...
  setActualSelection(arr) {
    ...
    if (first) {
      ...
      if (feature.layer == undefined) feature.layer = 0;

      this.sendMsg("classSelection", {
        descriptor: this.activeSoc.descriptorForIndex(feature.classIndex),
      });
      this.selectedIntersections = this.geom.getIntersections(first);

      if (this.mergingFirstPath) {
        if (
          this.mergingFirstPath != first &&
          this.geom.getIntersections(first)
        ) {
          this.mergePaths(this.mergingFirstPath, first);
        } else {
          this.sendMsg("alert", {
            message: "Merging cancelled: the polygons don't intersect.",
          });
        }
        this.mergingFirstPath = null;
      }
  ...
  saveData(ignoreUndo) {
    ...
    this.mainLayer.children.forEach((path) => {
      const polygon = path.segments.map((seg) => {
        const p = seg.point;
        return {
          x: p.x,
          y: p.y,
  ...
  drawAnnotations() {
    ...
    this.currentSample.objects.forEach((feature) => {
      ...
      this.setColor(
        path,
        this.activeSoc.colorForIndexAsHex(feature.classIndex)
      );
    });
    this.updateGeometry(false);
  ...
  repaint() {
    ...
    this.mainLayer.children.forEach((path) => {
      this.setColor(
        path,
        this.activeSoc.colorForIndexAsHex(path.feature.classIndex)
      );
    });
  }
  ...
  fixOrderingForOneItem() {
    ...
    let cursorItem,
      ...
      items = this.mainLayer.children.concat();
    items.some((item) => {
      if (!cursorItem) cursorItem = item;
        ...
        if ((item.feature.layer || 0) < (cursorItem.feature.layer || 0)) {
          ...
          (item.feature.layer || 0) > (cursorItem.feature.layer || 0)
        ) {
          cursorItem = item;
        }
      }
  ...
  flattenNonSimplePath(rpath) {
    if (rpath.children) {
      rpath.children.concat().forEach((npath) => {
        ...
        npath.feature = {
          classIndex: rpath.feature.classIndex,
          layer: rpath.feature.layer,
          polygon: [],
        };
    ...
    if (this.disabledComposition) {
      ...
      this.mainLayer.getChildren().forEach((path) => {
        if (!this.newPath || this.newPath != path) {
          ...
          if (!hideStroke) path.strokeColor = "white";
        }
      });
      /*
            if (this.newPath) {
  ...
  onZoom(zoomLvl) {
    ...
    this.sendMsg("zoomLevel", {
      value: Math.round(100 * zoomLvl * this.scaleFactor) / 100,
    });
  ...
  hitTesting(event) {
    const matchFunction = (ht) => {
      ...
      if (accept && this.pendingSegment)
        ...
        accept =
          accept &&
          (ht.item != this.pendingSegment.path ||
            (ht.segment &&
              ht.segment.index == 0 &&
        ...
        if (isoSegments) {
          ...
          if (polygons) {
            if (polygons[0] == ht.item) {
              ...
              while (!found && !seg.done) {
                if (
                  seg.value != ht.location.segment &&
                  seg.value.path == polygons[0]
                ) {
                  accept = false;
                  found = true;
                }
    ...
    this.hitTolerance = areas.tolerance = pointsAndLines.tolerance =
      7 / (this.scaleFactor * Paper.view.zoom);

    const hitPointsAndLines = Paper.project.hitTest(
      event.point,
      pointsAndLines
    );
    ...
    if (hitPointsAndLines) {
      ...
      if (hitPointsAndLines.type == "curve") {
        ...
        if (this.pendingSegment) {
          const isoSegments =
            this.isoSegments(hitPointsAndLines.segment) || new Set();
          let iter = isoSegments.values();
          let found = false;
          let seg = iter.next();
          while (!found && !seg.done) {
            if (
              seg.value != hitPointsAndLines.segment &&
...
*/
  setupMessages() {
    ...
    this.onMsg("selectAll", (args) => {
      this.mainLayer.children.forEach(
        (p) => (p.fullySelected = args.value == true)
      );
    });
    ...
    this.onMsg("layer-hide", (arg) => {
      ...
      this.mainLayer.children
        .filter(
          (pol) =>
            (arg.index == 0 && pol.feature.layer == undefined) ||
            pol.feature.layer == arg.index
        )
        .forEach((pol) => {
    ...
    this.onMsg("class-multi-select", (arg) => {
      ...
      this.setActualSelection(
        this.mainLayer.children.filter(
          (p) => p.feature.classIndex == arg.classIndex
        )
      );
    });
...
*/
  ...
  showStrokes(v) {
    ...
    if (v) this.setActualSelection(this.mainLayer.children);
    else this.clearActualSelection();
  }
  ...
  cancel(fullCancel) {
    ...
    if (Paper.tool.cancel) Paper.tool.cancel(fullCancel);
  }

  delete() {
    if (this.newPath) this.cancel(true);
    const editor = this;
  ...
  imageLoaded() {
    ...
    this.sourceImageData = ctx.getImageData(
      0,
      0,
      this.imageWidth,
      this.imageHeight
    ...
    this.raster = new Paper.Raster(
      image,
      new Paper.Point(this.imageWidth / 2, this.imageHeight / 2)
    );
    ...
    this.raster.onLoad = () => {
      ...
      let fileName = decodeURIComponent(
        url.parse(document.URL).path.replace("/edit/", "")
      );
  ...
  updateStats() {
    const pointCount = this.currentSample.objects.reduce(
      (acc, cur) => (acc += cur.polygon.length),
      0
    );
    let statLabel = this.currentSample.objects.length + " object";
    statLabel += this.currentSample.objects.length > 1 ? "s" : "";
    if (pointCount) statLabel += ", " + pointCount + " points";
    this.sendMsg("bottom-right-label", { message: statLabel });

    const classCounter = new Map();
    const layerCounter = new Map();
    [...Array(this.activeSoc.classesCount).keys()].forEach((v, k) =>
      classCounter.set(k, 0)
    );
    this.mainLayer.children.forEach((path) => {
      classCounter.set(
        path.feature.classIndex,
        classCounter.get(path.feature.classIndex) + 1
      );
      layerCounter.set(
        path.feature.layer || 0,
        layerCounter.get(path.feature.layer || 0) + 1 || 1
      );
    });
