...
export default class SsePolygonTool extends SseTool {
  ...
  onMouseMove(event) {
    ...
    if (event.modifiers.shift) {
      ...
      if (this.readyToClose) {
        ...
        this.editor.snap(
          this.editingPath.firstSegment.point,
          "red",
          "square",
          this.editingPath.firstSegment
        );
  ...
  cancel(fullCancel) {
    ...
    if (fullCancel) {
      if (this.editingPath) this.editingPath.remove();
  ...
  onMouseUp(event) {
    ...
    if (
      this.readyToClose &&
      this.lastMouseUp &&
      new Date().getTime() - this.lastMouseUp < this.doubleClickTiming
    ) {
      this.endPolygon();
    } else {
  ...
  onMouseDown(event) {
    ...
    if (this.editor.snapIndicator) {
      ...
      point = this.editor.rasterLayer.globalToLocal(
        this.editor.keepPointInsideRaster(event.point)
      );

    if (this.isDrawing) {
      if (this.editor.hitInfos && this.editor.hitInfos.type == "line") {
        const newSegment = this.editor.hitInfos.polygon.divideAt(
      ...
      this.editingFeature = {
        classIndex: this.editor.activeClassIndex,
        polygon: [],
      };
  ...
  copyFeature(feature) {
    return {
      ...
      polygon: feature.polygon.concat(),
      layer: feature.layer,
    };
  }
  ...
  endPath(newPath, feature) {
    ...
    if (
      newPath.segments.length < 10 ||
      newPath.bounds.width < 10 ||
      newPath.bounds.height < 10
    )
      simplifiedPoints = newPath.segments.map((s) => s.point);
    else
      simplifiedPoints = simplify(
        newPath.segments.map((s) => s.point),
        0.5,
        true
      );
  ...
  endPolygon() {
    ...
    let simplifiedPoints;
    if (
      this.editingPath.segments.length < 10 ||
      this.editingPath.bounds.width < 10 ||
      this.editingPath.bounds.height < 10
    )
      simplifiedPoints = this.editingPath.segments.map((s) => s.point);
    else
      simplifiedPoints = simplify(
        this.editingPath.segments.map((s) => s.point),
        0.5,
        true
      );
