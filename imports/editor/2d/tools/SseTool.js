...
export default class SseTools {
  ...
  activate() {
    if (Paper.tool && Paper.tool.cancel) Paper.tool.cancel(true);
    this.paperTool.activate();
    if (this.editor) SseGlobals.setCursor(this.cursor || "default");
  }
  ...
  viewDrag(event) {
    ...
    const deltaPixel = Paper.view
      .projectToView(event.point)
      .subtract(this.downPoint);
    const candidateCenter = this.downCenter.subtract(
      deltaPixel.divide(Paper.view.zoom)
    );
    candidateCenter.x = Math.max(this.editor.offsetX, candidateCenter.x);
    candidateCenter.x = Math.min(
      this.editor.viewWidth - this.editor.offsetX,
      candidateCenter.x
    );
    candidateCenter.y = Math.max(this.editor.offsetY, candidateCenter.y);
    candidateCenter.y = Math.min(
      this.editor.viewHeight - this.editor.offsetY,
      candidateCenter.y
    );
