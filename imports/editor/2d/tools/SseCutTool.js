...
export default class SseCutTool extends SseTool {
  ...
  avoidCrossings() {
    if (
      this.editor.selectorPath.getCrossings(this.editor.selectorPath).length > 0
    ) {
      this.cancel();
      return true;
    }
  ...
  onMouseDown(event) {
    ...
    const pointHit =
      ...
      (this.editor.hitInfos.type == "point" ||
        this.editor.hitInfos.type == "line");
    const hitOtherPolygon =
      pointHit &&
      this.fromSegment &&
      this.fromSegment.path != this.editor.hitInfos.polygon;
    ...
    if (!this.editor.selectorPath && pointHit) {
      ...
      if (this.editor.hitInfos.type == "point") {
        ...
        this.fromSegment = this.editor.hitInfos.polygon.divideAt(
          this.editor.hitInfos.location
        );
        if (!this.fromSegment) return;
        else this.segmentsFromDivision.add(this.fromSegment);
      }

      this.adjustedPolygon = this.fromSegment.path;
      this.editor.frontLayer.activate();
      this.editor.selectorPath = new Paper.Path([
        this.fromSegment,
        this.fromSegment,
      ]);
      ...
      if (this.editor.hitInfos.type == "point") {
        ...
        this.toSegment = this.editor.hitInfos.polygon.divideAt(
          this.editor.hitInfos.location
        );
        if (!this.toSegment) return;
        else this.segmentsFromDivision.add(this.toSegment);
      }
      this.avoidCrossings();
      this.finalize();
    }
  ...
  onMouseMove(event) {
    ...
    if (this.editor.selectorPath) {
      ...
      if (this.editor.hitInfos && this.editor.hitInfos.point)
        localPoint = this.editor.rasterLayer.globalToLocal(
          this.editor.hitInfos.point
        );
      else localPoint = this.editor.rasterLayer.globalToLocal(localPoint);
      ...
      if (this.editor.hitInfos.type == "polygon") {
        ...
        this.editor.snap(
          this.editor.rasterLayer.globalToLocal(this.editor.hitInfos.point),
          "white",
          "square"
        );
      }
  ...
  finalize() {
    if (this.editor.selectorPath) {
      if (
        this.fromSegment.next == this.toSegment ||
        this.fromSegment.previous == this.toSegment
      ) {
        const artificialPoint = new Paper.Point(
          (this.fromSegment.point.x + this.toSegment.point.x) / 2,
          (this.fromSegment.point.y + this.toSegment.point.y) / 2
        );
        this.adjustedPolygon.divideAt(
          this.adjustedPolygon.getNearestLocation(artificialPoint)
        );
      }
      ...
      const simplifiedPoints = simplify(
        this.editor.selectorPath.segments.map((s) => s.point),
        0.5,
        true
      );
      ...
      const previousPath = new Paper.Path(
        simplifiedPoints.concat(previousSide)
      );
