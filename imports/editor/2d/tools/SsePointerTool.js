...
export default class SsePointerTool extends SseTool {
  ...
  onMouseDown(event) {
    ...
    if (
      !this.editor.snapPoint &&
      this.editor.currentSample.objects.length > 0
    ) {
    ...
    // so let's divide the destination and create a new segment
      ...
      this.snappedSegment = this.editor.hitInfos.polygon.divideAt(
  ...
  onMouseMove(event) {
    ...
    if (this.editor.hitInfos) {
      ...
      if (
        this.editor.hitInfos.type == "polygon" ||
        ...
        else SseGlobals.setCursor("crosshair");

        if (this.draggingSegments)
          this.draggingSegments.forEach((s) =>
            this.editor.hitExcluded.delete(s)
          );
      ...
      // Point pending selection abd snapping
        ...
        this.editor.setPendingSelection([
          this.editor.hitInfos.segment,
          this.editor.hitInfos.segment.path,
        ]);
        SseGlobals.setCursor("pointer");
        this.editor.snap(
          this.editor.rasterLayer.globalToLocal(this.editor.hitInfos.point),
      ...
      if (
        !this.candidatePendingPolygon ||
        !this.candidatePendingPolygon.contains(
          this.editor.rasterLayer.globalToLocal(event.point)
        )
      ) {
  ...
  onMouseUp(event) {
    ...
    if (this.editor.selectorPath) {
      ...
      hittedSegments = hittedSegments.filter(
        (i) => i.type == "segment" && i.item != this.editor.selectorPath
      );
      ...
      hittedSegments.forEach((seg) => {
        ...
        if (!d1 || !d2) {
          ...
          if (this.editor.selectorPath.contains(d1.next.point)) {
            let p = d1.next;
            while (p != d2) {
              if (this.editor.selectorPath.contains(p.point)) toSelect.add(p);
              p = p.next;
            }
          } else {
            let p = d1.previous;
      ...
      if (toSelect.size > 0) {
        Array.from(toSelect).map((itm) => {
          if (itm.path) toSelect.add(itm.path);
        });
