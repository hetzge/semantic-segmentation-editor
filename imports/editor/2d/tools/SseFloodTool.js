...
export default class SseFloodTool extends SseTool {
  ...
  process(pt) {
    ...
    if (!pt && this.downPoint) pt = this.downPoint;
    else if (!pt && !this.downPoint) return;
  ...
  baseLayer() {
    return this.editor.mainLayer.children.length == 0 ||
      this.editor.mainLayer.children.filter((x) => x.visible).length == 0
      ? this.editor.mainLayer
      : this.editor.frontLayer;
  }
  ...
  flood(x, y, thr, rad) {
    ...
    if (cs[0]) {
      let pts = cs[0].points;
      pts = pts.map((pt) => ({ x: pt.x + 0.5, y: pt.y + 0.5 }));
