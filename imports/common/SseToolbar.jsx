...
export default class SseToolbar extends React.Component {
  ...
  renderCommand(name) {
    ...
    if (!commandDesc) return null;
      ...
      return (
        <div
          className={
            "sse-command vflex flex-align-items-center " + this.state[name]
          }
        ...
        >
          <IconButton
            onClick={(e) =>
              this.sendMsg(commandDesc.actionMessage, {
          ...
          <span className="title">
            {commandDesc.legend || commandDesc.shortcut}
          </span>
        </div>
      );
    }
  ...
  renderMiniCommand(name, cssClasses = "") {
    ...
    if (!commandDesc) {
      ...
      return (
        <div
          className={
            "sse-command mini vflex flex-align-items-center " +
            this.state[name] +
            " " +
            cssClasses
          }
        ...
        >
          <div className="hflex flex-align-items-center">
            {commandDesc.legend ? (
              ...
              <IconButton
                onClick={(e) => this.sendMsg(commandDesc.actionMessage)}
              >
                <Icon />
                <span>{commandDesc.legend}</span>
              </IconButton>
    ...
    return (
      ...
      >
        <FormControlLabel
          ...
          label={
            commandDesc.title +
            (commandDesc.shortcut ? " (" + commandDesc.shortcut + ")" : "")
          }
        />
      </FormGroup>
    );
  ...
  handleCheckbox(commandName) {
    ...
    this.sendMsg(commandDesc.actionMessage.replace("-checkbox", ""), {
      value: cb.checked,
    });
  ...
  addCommand(
    ...
    if (isToggle) {
      if (typeof isToggle == "boolean") this.toggleSet.add(command);
    }
      ...
      if (isToggle) {
        if (typeof isToggle == "number") {
          ...
          this.pendingState[name] =
            this.state[name] == enabled ? highlighted : enabled;
        }
        this.invalidate();
      }
    });
    ...
    if (shortcut) {
      if (!actionMessage.endsWith("-checkbox"))
        Mousetrap.bind(shortcut.toLowerCase(), () =>
          this.sendMsg(actionMessage)
        );
      else Mousetrap.bind(shortcut.toLowerCase(), () => $("#" + name).click());
    }
  }
