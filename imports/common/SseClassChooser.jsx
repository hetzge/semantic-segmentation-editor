...
export default class SseClassChooser extends SseToolbar {
    ...
    this.onMsg("editor-ready", (arg) => {
      if (arg && arg.value && arg.value.socName)
        this.sendMsg("active-soc", {
          value: this.classesSetByName.get(arg.value.socName),
        });
      else this.sendMsg("active-soc", { value: this.classesSets[0] });
    });
  ...
  displayAll() {
    if (this.state) {
      Object.keys(this.state).forEach((k) => {
        if (
          k.toString().startsWith("mute") ||
          k.toString().startsWith("solo")
        ) {
          delete this.state[k];
        }
      });
  ...
  muteOrSolo(name, argument, idx) {
    if (
      this.state.counters[argument.classIndex] ||
  ...
  changeClassesSet(name) {
    ...
    const usedClasses = Object.keys(this.state.counters).filter(
      (x) => this.state.counters[x] > 0
    );
    const missing = [];
          ...
          <div className="vflex">
            ...
            <div className="hflex w100 wrap">
              {this.classesSets.map((cset) => (
                ...
                >
                  {cset.name +
                    (cset.name == this.state.soc.name ? " (current)" : "")}
                </Button>
              ))}
            </div>
          </div>
  ...
  render() {
    ...
    return (
      <div
        className="sse-class-chooser vflex scroller"
      ...
      >
        {this.state.soc.descriptors.map((objDesc, idx) => {
          ...
          return (
            <div
              className="hflex flex-align-items-center no-shrink"
              key={objDesc.label}
            >
              <ChevronRight
                className="chevron"
              ...
              <Button
                ...
                onDoubleClick={() =>
                  this.sendMsg("class-multi-select", { name: objDesc.label })
                }
                onClick={() => {
              ...
              >
                <div className="hflex flex-align-items-center w100">
                  {this.getIcon(objDesc)}
                  {objDesc.label}
                </div>
                <sup>
                  {this.state.counters[objDesc.classIndex] > 0
                    ? this.state.counters[objDesc.classIndex]
                    : ""}
                </sup>
              </Button>
              {this.props.mode == "3d" ? (
