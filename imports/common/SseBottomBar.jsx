...
class SseBottomBar extends React.Component {
  ...
  handleClose = () => {
    ...
    this.currentSample.tags = this.tagsStringToArray(
      $("#tagField").val()
    ).filter((x) => x);
  ...
  addTag(t) {
    this.setState({
      tags: this.tagsStringToArray(
        this.tagsArrayToString(this.state.tags) + "," + t
      ),
    });
  }
      ...
      >
        <div className="hflex">
          <div className="grow hflex flex-align-items-center children-margin">
            ...
            <button className="sse-button" onClick={() => this.handleOpen()}>
              Set Tags
            </button>

            {this.state.tags.map((t) => (
              <div key={t} className="sse-tag">
                {t}
              </div>
            ))}
          </div>
              ...
              <div className="hflex w100 wrap">
                {this.props.appProps.tags.value.map((t) => (
                  <div
                    ...
                    key={t}
                    className="sse-tag m5-bottom cursor-pointer"
                  >
                    {t}
                  </div>
                ))}
...
export default withTracker((props) => {
  ...
  if (!appProps.tags) appProps.tags = { value: [] };
